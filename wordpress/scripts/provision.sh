...
THEME_NAME=${THEME_NAME:="starterpack"}
...
if ! $WPCLI core is-installed --quiet &> /dev/null; then
	...
	if [ ! -z "${WORDPRESS_ENV}" ] && [ "${WORDPRESS_ENV}" = "dev" ]; then # we are on local dev environment (in docker)
		...
		$WPCLI core install --url="http://localhost" --title="starterpack" --admin_user="superhuit" --admin_password="superhuit" --admin_email="tech+starterpack@superhuit.ch" --quiet &> /dev/null
		echo "✔"
...
$WPCLI theme uninstall twentysixteen --quiet &> /dev/null
$WPCLI theme uninstall twentyseventeen --quiet &> /dev/null
$WPCLI theme uninstall twentynineteen --quiet &> /dev/null
