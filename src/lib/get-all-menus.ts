...
export const formatter = (props: unknown): Menus => {
	...
	const newMenus = Object.keys(menus).reduce((acc, location) => {
		return {
			...acc,
			[location]: {
				...menus[location as keyof typeof menus],
		...
		header: { items: Array<MenuItemType> };
		footer: { items: Array<MenuItemType> };
		legal: { items: Array<MenuItemType> };
		social: { items: Array<MenuItemType> };
	};
